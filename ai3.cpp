...
int AI::sort_inferno(Pos pos){
    ...
    ret-=getalldamage(pos);
    ret-=cube_distance(pos,enemy_center);
...
Pos AI::start_sword(Unit a,vector<Pos> reach_pos_list,vector<Unit> enemy_list){
    ...
    for(auto pos:reach_pos_list){
        ...
        if(pos==r3&&can_sword(a,r3,enemy_list)&&level>2){
            ret=r3;
            level=2;
            continue;
        }
        if(pos==l3&&can_sword(a,l3,enemy_list)&&level>3){
            ret=l3;
            level=3;
            continue;
        }
        ...
        if(can_sword(a,pos,enemy_list)&&level>4){
            level=5;
            int now=sort_sword(pos);
            if(now>val1){
                val1=now;
...
Pos AI::start_priest(Unit a,vector<Pos> reach_pos_list,vector<Unit> enemy_list){
    ...
    for(auto pos:reach_pos_list){
        ...
        if(pos==c2&&can_priest(a,c2,enemy_list)&&level>1){
            ret=c2;
            level=1;
            continue;
        }
        if(pos==l2&&can_priest(a,l2,enemy_list)&&level>2){
            ret=l2;
            level=2;
            continue;
        }
        ...
        if(can_priest(a,pos,enemy_list)&&level>6){
            level=7;
            int now=sort_priest(pos);
            if(now>val1){
                val1=now;
...
Pos AI::start_archer(Unit a,vector<Pos> reach_pos_list,vector<Unit> enemy_list){
    ...
    for(auto pos:reach_pos_list){
        ...
        if(pos==rr4&&can_archer(a,rr4,enemy_list)&&level>1){
            ret=rr4;
            level=1;
            continue;
        }
